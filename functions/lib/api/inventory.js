...
// ============================================================================
export function calculateFreshnessScore(landingDate, storageMethod, storageTempCelsius, initialGrade = 'a') {
    const now = Date.now();
...
export function calculateGradeExpiry(landingDate, initialGrade) {
    if (initialGrade !== 'sushi')
        return undefined;
 ...
 */
export const createInventoryBatch = functions.https.onCall(async (request) => {
    ...
    try {
        ...
        const freshnessScore = calculateFreshnessScore(data.landingDate, data.storageMethod, data.storageTempCelsius, data.initialGrade);
 ...
 */
export const updateInventoryBatch = functions.https.onCall(async (request) => {
    const data = request.data;
    const context = request;
    if (!request.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    ...
    try {
        ...
        if (updates.storageMethod || updates.storageTempCelsius !== undefined) {
            updateData.freshnessScore = calculateFreshnessScore(batch.landingDate, updates.storageMethod || batch.storageMethod, updates.storageTempCelsius !== undefined ? updates.storageTempCelsius : batch.storageTempCelsius, updates.currentGrade || batch.currentGrade);
        }
        if (updates.notes) {
 ...
 */
export const convertBatchToListing = functions.https.onCall(async (request) => {
    const data = request.data;
    const context = request;
    if (!request.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    ...
    if (quantityKg > availableKg) {
        throw new functions.https.HttpsError('invalid-argument', `Cannot list ${quantityKg}kg. Only ${availableKg}kg available.`);
    }
    ...
    try {
        ...
        const newStatus = newListedKg >= batch.remainingKg ? 'listed' : 'partially_sold';
 ...
 */
export const getInventoryBatches = functions.https.onCall(async (request) => {
    const data = request.data;
    const context = request;
    if (!request.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    ...
    try {
        let query = db.collection('inventoryBatches')
 ...
 */
export const getInventoryMovements = functions.https.onCall(async (request) => {
    const data = request.data;
    const context = request;
    if (!request.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
 ...
 */
export const discardInventoryBatch = functions.https.onCall(async (request) => {
    const data = request.data;
    const context = request;
    if (!request.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    ...
    if (unsoldKg <= 0) {
        throw new functions.https.HttpsError('failed-precondition', 'No unsold inventory to discard');
    }
    try {
        const now = admin.firestore.Timestamp.now();
        // Update batch
 ...
 */
export const getInventoryDashboard = functions.https.onCall(async (request) => {
    if (!request.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const userId = request.auth.uid;

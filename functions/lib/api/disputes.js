 ...
 */
export const fileDispute = functions.https.onCall(async (request) => {
    ...
    try {
        ...
        if (!pickupTime) {
            throw new functions.https.HttpsError('failed-precondition', 'Cannot dispute order that has not been picked up or delivered');
        }
        const hoursSincePickup = (now.toMillis() - pickupTime.toMillis()) / (1000 * 60 * 60);
        if (hoursSincePickup > DISPUTE_FILING_WINDOW_HOURS) {
            throw new functions.https.HttpsError('failed-precondition', `Dispute must be filed within ${DISPUTE_FILING_WINDOW_HOURS} hours of pickup/delivery`);
        }
        ...
        if (!existingDispute.empty) {
            throw new functions.https.HttpsError('already-exists', 'Order already has an open dispute');
        }
 ...
 * Get dispute details
 */
export const getDispute = functions.https.onCall(async (request) => {
    const data = request.data;
    const context = request;
    if (!request.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
 ...
 */
export const addDisputeMessage = functions.https.onCall(async (request) => {
    const data = request.data;
    const context = request;
    if (!request.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
 ...
 */
export const resolveDispute = functions.https.onCall(async (request) => {
    const data = request.data;
    const context = request;
    if (!request.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
 ...
 */
export const appealDispute = functions.https.onCall(async (request) => {
    const data = request.data;
    const context = request;
    if (!request.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    ...
    try {
        ...
        const canAppeal = (isBuyer && dispute.status === 'resolved_seller_favor') ||
            (!isBuyer && dispute.status === 'resolved_buyer_favor') ||
            (dispute.status === 'compromise');
        if (!canAppeal) {
            throw new functions.https.HttpsError('failed-precondition', 'Cannot appeal this resolution');
        }
        ...
        if (dispute.resolution?.resolvedAt) {
            ...
            if (hoursSinceResolution > 48) {
                throw new functions.https.HttpsError('failed-precondition', 'Appeal window (48 hours) has expired');
            }
        }
        await disputeRef.update({
            status: 'appealed',
 ...
 */
export const getMyDisputes = functions.https.onCall(async (request) => {
    const data = request.data;
    const context = request;
    if (!request.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
 ...
 */
export const getAllDisputes = functions.https.onCall(async (request) => {
    const data = request.data;
    const context = request;
    if (!request.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
...
async function holdSellerPayout(orderId) {
...
async function processRefund(orderId, amount, dbOrTransaction) {
    ...
    if ('set' in dbOrTransaction) {
        // It's a transaction
        dbOrTransaction.set(refundRef, refundData);
    }
    else {
...
async function adjustSellerPayout(orderId, resolution, refundAmount, dbOrTransaction) {
    ...
    if ('get' in dbOrTransaction && 'update' in dbOrTransaction) {
        // It's a transaction
        const transaction = dbOrTransaction;
...
async function attemptAutoResolution(disputeId, data) {
    const now = admin.firestore.Timestamp.now();
    ...
    if (data.disputeType === 'quantity_short' && data.actualQuantityKg && data.evidencePhotos?.length >= 1) {
        ...
        if (shortPercent > AUTO_RESOLUTION_THRESHOLD_SHORT) {
            ...
            // Process refund
            await processRefund(data.orderId, refundAmount, db);
            // Adjust seller payout
            await adjustSellerPayout(data.orderId, 'partial_refund', refundAmount, db);
            // Notify parties
...
async function updateSellerDisputeMetrics(sellerId, resolution) {
    if (!sellerId)
        return;
...
async function recalculateTier(sellerId) {
...
// ============================================================================
async function notifyDisputeOpened(recipientId, disputeId, orderId, isBuyerFiling) {
    try {
...
async function notifyNewMessage(recipientId, disputeId, senderType) {
    try {
        const { notifyUser } = await import('../utils/notifications.js');
        await notifyUser({
            userId: recipientId,
...
async function notifyResolution(recipientId, disputeId, resolution, isBuyer) {
    try {
        const { notifyUser } = await import('../utils/notifications.js');
...
async function notifyAdminOfAppeal(disputeId, appellantId, reason) {
    try {

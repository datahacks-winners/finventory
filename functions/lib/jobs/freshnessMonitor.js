    ...
    try {
        ...
        for (const batchDoc of activeBatches.docs) {
            try {
                ...
                const newScore = calculateFreshnessScore(batch.landingDate, batch.storageMethod, batch.storageTempCelsius, batch.currentGrade);
 ...
 */
async function sendDowngradeNotification(sellerId, batchId, batch) {
    try {
 ...
 */
async function sendBuyerGradeChangeNotification(buyerId, orderId) {
    try {
        const { notifyUser } = await import('../utils/notifications.js');
        await notifyUser({
            userId: buyerId,
 ...
 */
async function sendExpiredNotification(sellerId, batchId, batch) {
    try {
        const { notifyUser } = await import('../utils/notifications.js');
        await notifyUser({
            userId: sellerId,

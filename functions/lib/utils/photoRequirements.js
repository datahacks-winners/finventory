...
export const PHOTO_REQUIREMENTS_BY_GRADE = {
    sushi: {
        minPhotos: 3,
        maxPhotos: 8,
...
export function validateListingPhotos(grade, photos) {
...
export function getPhotoRequirementsForGrade(grade) {
    return PHOTO_REQUIREMENTS_BY_GRADE[grade.toLowerCase()] || null;
}
export function getPhotoTypeDescription(grade, type) {
    const requirements = PHOTO_REQUIREMENTS_BY_GRADE[grade.toLowerCase()];
    if (!requirements)
        return '';
 ...
 */
export const validatePhotos = functions.https.onCall(async (request) => {
 ...
 */
export const getPhotoRequirements = functions.https.onCall(async (request) => {
    const data = request.data;
    const context = request;
    if (!request.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
...
// ============================================================================
export async function trackPhotoCompliance(db, sellerId, listingId, validation) {
...
export async function analyzePhotoWithAI(_imageUrl) {
 ...
 */
export const analyzeListingPhoto = functions.https.onCall(async (request) => {
    const data = request.data;
    const context = request;
    if (!request.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');

...
export async function notifyUser(payload) {
    ...
    await sendPushNotification(payload.userId, payload.title, payload.body, payload.data);
}
/**
 * Notify buyer of standing order match
 */
export async function notifyStandingOrderMatch(buyerId, listingId, species, grade) {
    await notifyUser({
        userId: buyerId,
 ...
 */
export async function notifyNewOrder(sellerId, orderId, listingTitle) {
    await notifyUser({
        userId: sellerId,
        type: 'order_update',
 ...
 */
export async function notifyOrderStatusUpdate(buyerId, orderId, status) {

...
export type VerificationTier = 'unverified' | 'bronze' | 'silver' | 'gold' | 'platinum'

export interface VerificationDocument {
  id: string
  sellerId: string
  type: 'business_license' | 'health_permit' | 'tax_id' | 'liability_insurance' | 'background_check'
...
export interface SellerVerification {
  ...
  verifications: {
    email: { verified: boolean; at?: admin.firestore.Timestamp }
    phone: { verified: boolean; at?: admin.firestore.Timestamp }
    businessLicense: { verified: boolean; documentId?: string; at?: admin.firestore.Timestamp }
    healthPermit: { verified: boolean; documentId?: string; at?: admin.firestore.Timestamp }
    taxId: { verified: boolean; at?: admin.firestore.Timestamp }
    bankAccount: { verified: boolean; at?: admin.firestore.Timestamp }
    physicalLocation: { verified: boolean; at?: admin.firestore.Timestamp; method?: 'mail' | 'video' | 'agent' }
  }
  ...
  // Background check (for higher tiers)
  backgroundCheck?: {
    provider: string
  ...
  hardLimitSetAt?: admin.firestore.Timestamp
...
export function calculateTierScore(verification: SellerVerification): number {
  let score = 0
  const v = verification.verifications
  
  // Identity verifications (40 points max)
  if (v.email.verified) score += 5
  if (v.phone.verified) score += 5
  if (v.businessLicense.verified) score += 10
  if (v.healthPermit.verified) score += 10
  if (v.taxId.verified) score += 5
  if (v.bankAccount.verified) score += 5
  
  // Location verification (10 points)
  if (v.physicalLocation.verified) score += 10
  
  // Performance metrics (30 points max)
  const m = verification.metrics
  if (m.totalOrders >= 10) score += 5
  if (m.totalOrders >= 50) score += 5
  if (m.avgRating >= 4.5) score += 10
  if (m.disputeRate < 0.02) score += 5
  if (m.onTimePickupRate > 0.95) score += 5
  
  // Additional (20 points)
  if (verification.backgroundCheck?.status === 'clear') score += 10
  if (verification.liabilityInsurance) score += 10
  
  return Math.min(100, score)
}

export function getTierFromScore(score: number): VerificationTier {
  ...
  async (_, context) => {
    ...
    try {
      ...
      if (!verification.exists) {
        ...
        const initialVerification: Omit<SellerVerification, 'sellerId'> = {
          tier: 'unverified',
          tierScore: 0,
          verifications: {
      ...
      const data = verification.data() as SellerVerification
    ...
    try {
      ...
      if (!existingDocs.empty) {
        throw new functions.https.HttpsError(
          'already-exists',
          `You already have a ${data.type} document pending or verified`
        )
      }
      ...
      const verificationUpdate: FirebaseFirestore.DocumentData = {}
...
export const reviewVerificationDocument = functions.https.onCall(
  async (data: {
    ...
    try {
      ...
      const docData = doc.data() as VerificationDocument
...
export const checkCanCreateListing = functions.https.onCall(
  async (data: { quantityKg: number }, context) => {
    ...
    try {
      ...
      const v = verification.data() as SellerVerification
...
async function recalculateSellerTier(sellerId: string) {
  const verification = await db.collection('sellerVerifications').doc(sellerId).get()
...
async function notifyAdminOfPendingDocument(sellerId: string, type: string, documentId: string) {
  try {
    // Get all admins
...
async function notifySellerOfVerification(sellerId: string, type: string, approved: boolean, reason?: string) {
  try {
    const { notifyUser } = await import('../utils/notifications.js')
...
async function notifySellerOfTierChange(sellerId: string, oldTier: string, newTier: string) {
  try {
    const { notifyUser } = await import('../utils/notifications.js')
...
async function notifySellerOfHardLimit(sellerId: string, reason: string) {
  try {
    const { notifyUser } = await import('../utils/notifications.js')

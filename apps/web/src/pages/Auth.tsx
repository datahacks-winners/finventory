...
export default function Auth() {
  ...
  const { signIn, signUp, signInWithGoogle, signInWithApple, error: authError, loading: _authLoading } = useAuth()
          ...
          {tab === 'login' && (
            <div className="bg-white rounded-3xl shadow-sm border border-black/5 p-8 md:p-10">
              <div className="mb-6 flex flex-col gap-3">
                <button onClick={async () => { try { await signInWithGoogle(); navigate(from, { replace: true }); } catch { /* error shown via context */ } }} className="w-full flex items-center justify-center gap-3 py-4 px-6 rounded-full border border-slate-200 bg-white hover:bg-slate-50 transition-all font-bold text-slate-700 text-sm shadow-sm">
                ...
                <button onClick={async () => { try { await signInWithApple(); navigate(from, { replace: true }); } catch { /* error shown via context */ } }} className="w-full flex items-center justify-center gap-3 py-4 px-6 rounded-full border border-slate-200 bg-black hover:bg-slate-900 transition-all font-bold text-white text-sm shadow-sm">
